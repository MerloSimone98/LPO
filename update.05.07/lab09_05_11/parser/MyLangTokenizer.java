...
public class MyLangTokenizer implements Tokenizer {
	...
	static { // initialization of the symbol and keyword tables: symbols and keywords are singleton lexical categories 
		...
		symbols.put("[", OPEN_SQR);
		symbols.put("]", CLOSE_SQR);
	...
	private void semanticAnnotation() { // required for num or bool literals
		switch (tokenType) {
			case NUM -> intValue = Integer.decode(tokenString);
			case BOOL -> boolValue = Boolean.parseBoolean(tokenString);
			default -> {
			} // no other annotations required
		}
	}
