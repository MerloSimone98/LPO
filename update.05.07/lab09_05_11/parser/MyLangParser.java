...
import static lab09_05_11.parser.TokenType.*;
...
public class MyLangParser implements Parser {
	...
	private Exp parseAtom() throws ParserException {
		...
		case FST -> parseFst();
		case SND -> parseSnd();
		case OPEN_SQR -> parseVector();
		default -> unexpectedTokenError();
		};
	}

	// parses vectors of numbers
	private Exp parseVector() throws ParserException{
		consume(OPEN_SQR); //could be skippable and replaced with nextToken() because OPEN_SQR should be already recognized
		final var leftExp = parseExp();
		consume(STMT_SEP);
		final var rightExp = parseExp();
		consume(CLOSE_SQR);
		return new Vector(leftExp, rightExp);
	}
